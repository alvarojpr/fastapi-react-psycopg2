...
export function App() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/simplesmenteget?email=${email}`, { // fetch é para fazer requisições HTTP
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        const responseData = await response.json(); // Transforma a resposta em JSON
